...
+ (void) clearSensitiveData : (NSMutableData *) data : (NSUInteger) length {
    
    NSMutableString *temporaryValue = [[NSMutableString alloc] initWithString:@""];
    
    for (int i=0; i < length; i++) {
        [temporaryValue  appendFormat:@"%c", (unsigned char)arc4random()];
    }
  //  [data replaceBytesInRange:NSMakeRange(0, length - 1) withBytes:temporaryValue];
}
...
+ (NSString *)passwordForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
    NSMutableData *data = [self passwordDataForService:service account:account error:error];
	if (data.length > 0) {
		NSString *string = [[NSString alloc] initWithData:(NSData *)data encoding:NSUTF8StringEncoding];
        [self clearSensitiveData:data :data.length];
...
+ (NSMutableData *)passwordDataForService:(NSString *)service account:(NSString *)account {
    return [self passwordDataForService:service account:account error:nil];
}


+ (NSMutableData *)passwordDataForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
    OSStatus status = SSKeychainErrorBadArguments;
	if (!service || !account) {
		if (error) {
			*error = [NSError errorWithDomain:kSSKeychainErrorDomain code:status userInfo:nil];
...
#if __has_feature(objc_arc)
	return (__bridge_transfer NSMutableData *)result;
#else
    return [(NSMutableData *)result autorelease];
#endif
}
...
//    NSMutableData *data = [password dataUsingEncoding:NSUTF8StringEncoding];
    NSMutableData *data = [NSMutableData dataWithBytes:[password UTF8String] length:[password
                                                                                  length]];
    BOOL res = [self setPasswordData:data forService:service account:account error:error];
    [self clearSensitiveData:data: [password length]];
    return res;
}
